// Generic production seeder: node docs/jee-build/seed.mjs <key>
// Parses docs/<key>_seed.sql and writes it via the Supabase service-role
// client, in FK-safe order. Extended from docs/neet-build/seed.mjs to also
// parse questions.answer_type and BOTH question_answers shapes (MCQ's
// correct_option_id vs numeric's correct_numeric_answer — gen.mjs writes a
// different column list per shape, detected here by INSERT statement text).
import { readFileSync } from "node:fs";
import { config } from "dotenv";
import { createClient } from "@supabase/supabase-js";

const key = process.argv[2];
if (!key) { console.error("Usage: node docs/jee-build/seed.mjs <key>"); process.exit(1); }
...
for (const stmt of statements) {
  ...
  if (stmt.includes("question_papers")) {
    ...
    questions.push({
      ...
      subject: nullableStr(fields[4]), topic: nullableStr(fields[5]), answer_type: fields[6],
    });
    ...
    // Two possible column lists (see gen.mjs): MCQ writes
    // (question_id, correct_option_id, explanation, source_note); numeric
    // writes (question_id, correct_numeric_answer, explanation, source_note).
    // Distinguish by the statement's own column list, not by guessing from
    // field shape.
    const isNumeric = stmt.includes("correct_numeric_answer");
    if (isNumeric) {
      answers.push({ question_id: fields[0], correct_numeric_answer: Number(fields[1]), explanation: fields[2], source_note: fields[3] });
    } else {
...
if (options.length > 0) await bulkInsert("question_options", options);
