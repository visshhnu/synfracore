// Generic collision checker: node docs/healthcare-build/collision_check.mjs <key>
// Checks every UUID in docs/<key>_seed.sql against all live UUIDs
// currently in production (question_papers/questions/question_options) --
// which by construction includes every previously-seeded paper, any cert.
import { readFileSync } from "node:fs";
import { config } from "dotenv";
import { createClient } from "@supabase/supabase-js";

const key = process.argv[2];
if (!key) { console.error("Usage: node docs/healthcare-build/collision_check.mjs <key>"); process.exit(1); }
...
console.log(`New UUIDs in ${key}:`, newUuids.size);
