...
// paper: node docs/healthcare-build/check-stems.mjs <key>
// <key> examples: "ccs1".."ccs6", "cpt1".."cpt6" -- the registry file is
// scoped by cert prefix (ccs-stem-registry.json / cpt-stem-registry.json)
// since the two cert tracks share no real subject-matter overlap risk.
import { readFileSync, writeFileSync, existsSync } from "node:fs";

const key = process.argv[2];
if (!key) { console.error("Usage: node docs/healthcare-build/check-stems.mjs <key>"); process.exit(1); }

const certPrefix = key.replace(/[0-9]+$/, "");
const REGISTRY_PATH = `d:/synfracore/docs/healthcare-build/${certPrefix}-stem-registry.json`;
...
console.log(`\nPASS -- ${key}'s ${thisPaperStems.length} stems registered. Registry (${certPrefix}) now covers: ${Object.keys(registry).join(", ")}.`);
