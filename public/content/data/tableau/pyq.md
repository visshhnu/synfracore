...
**Framing note:** Unlike GATE/JEE/banking exams elsewhere on this platform, Tableau's certification exams (administered by Salesforce/Tableau via Pearson VUE) are proctored and confidential — there's no public past-paper archive to draw from. What follows are practice questions built directly from the **published exam guide domains** for the two certifications that actually test this material — **Tableau Desktop Specialist** (renamed **Salesforce Certified Tableau Desktop Foundations** as part of a July 2025 migration to the Trailhead Academy platform, which also dropped the price from $100 to $75; 45 questions, 60 minutes, no expiry — `(needs verification — recheck against current source: Salesforce periodically revises exam names/pricing on this platform)`) and **Tableau Certified Data Analyst** (price ~$250 as of this writing, 40-45 MCQ/multi-select plus 8-10 hands-on lab items, 120 minutes, 2-year validity) — rather than invented "leaked" questions.

**Correction to this course's own existing certification material:** `certification.md` and `fundamentals.md` both list outdated pricing ($250 for Desktop Specialist, and reference "Tableau Desktop Certified Associate" as a current credential). That associate-tier exam and the old Certified Professional tiers were retired September 30, 2021. The corrected current ladder is: Desktop Foundations (formerly Desktop Specialist, $75, never expires) → Certified Data Analyst (~$250, 2-year validity) → Server Certified Associate (~$250) → Tableau Certified Consultant (professional tier). Worth updating in the source files.
