...
function renderMarkdown(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("```")) {
      ...
      elements.push(
        <div key={`code-${i}`} style={{ margin: "12px 0", borderRadius: "10px", overflow: "hidden", border: "1px solid var(--border)" }}>
    ...
    if (line.startsWith("# ")) elements.push(<h1 key={i} style={{ fontSize: "20px", fontWeight: 700, margin: "16px 0 8px", color: "var(--text-1)" }}>{line.slice(2)}</h1>);
    else if (line.startsWith("## ")) elements.push(<h2 key={i} style={{ fontSize: "17px", fontWeight: 700, margin: "14px 0 6px", color: "var(--text-1)" }}>{line.slice(3)}</h2>);
    else if (line.startsWith("### ")) elements.push(<h3 key={i} style={{ fontSize: "15px", fontWeight: 700, margin: "12px 0 4px", color: "var(--text-1)" }}>{line.slice(4)}</h3>);
    else if (line.match(/^[-*] /)) elements.push(
      <div key={i} style={{ display: "flex", gap: "8px", margin: "4px 0" }}>
        <span style={{ color: "#60A5FA", flexShrink: 0 }}>•</span>
        <span style={{ color: "var(--text-3)", fontSize: "14px", lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: formatInline(line.slice(2)) }} />
      </div>
    );
      ...
      elements.push(
        <div key={i} style={{ display: "flex", gap: "10px", margin: "4px 0" }}>
          ...
          <span style={{ color: "var(--text-3)", fontSize: "14px", lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: formatInline(line.replace(/^\d+\. /, "")) }} />
        </div>
      );
    }
    else if (line.startsWith("> ")) elements.push(<div key={i} style={{ borderLeft: "3px solid #3B82F6", background: "rgba(59,130,246,0.05)", padding: "8px 14px", margin: "8px 0", borderRadius: "0 6px 6px 0" }}><p style={{ color: "var(--text-3)", fontSize: "13px", margin: 0 }}>{line.slice(2)}</p></div>);
    else if (line === "---") elements.push(<hr key={i} style={{ border: "none", borderTop: "1px solid var(--border)", margin: "16px 0" }} />);
    else if (line.trim() === "") elements.push(<div key={i} style={{ height: "6px" }} />);
    else elements.push(<p key={i} style={{ color: "var(--text-3)", fontSize: "14px", lineHeight: 1.75, margin: "3px 0" }} dangerouslySetInnerHTML={{ __html: formatInline(line) }} />);
    i++;
  }
  return elements;
}
...
Always provide: accurate technical answers, working code examples in fenced blocks with language tags, best practices, and clear structure using markdown.`,
  ...
  return (
    <div style={{ height: "calc(100vh - 65px)", display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div style={{ borderBottom: "1px solid var(--border)", padding: "14px 24px", display: "flex", alignItems: "center", gap: "14px", background: "var(--bg-1)", flexShrink: 0 }}>
      ...
      <div style={{ flexGrow: 1, overflowY: "auto", padding: "24px" }}>
        {messages.length === 0 ? (
          <div style={{ maxWidth: "740px", margin: "0 auto" }}>
            ...
            <p style={{ color: "var(--text-4)", fontSize: "11px", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "12px" }}>Suggested questions</p>
            <div style={{ display: "grid", gap: "8px", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))" }}>
              {suggestions.map((s) => (
                <button key={s} onClick={() => sendMessage(s)} style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: "11px", padding: "11px 15px", textAlign: "left", color: "var(--text-3)", fontSize: "13px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", gap: "10px" }}
          ...
          <div style={{ maxWidth: "740px", margin: "0 auto" }}>
            ...
            {loading && (
              <div style={{ display: "flex", gap: "12px", marginBottom: "22px" }}>
                ...
                <div style={{ padding: "13px 17px", borderRadius: "4px 16px 16px 16px", background: "var(--bg-2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "8px" }}>
                  <Loader2 size={15} style={{ color: "var(--text-4)", animation: "spin 1s linear infinite" }} />
                  <span style={{ color: "var(--text-4)", fontSize: "13px" }}>Thinking...</span>
                </div>
              </div>
            )}
            <div ref={bottomRef} />
      ...
      {/* Input */}
      <div style={{ borderTop: "1px solid var(--border)", padding: "14px 24px", background: "var(--bg-1)", flexShrink: 0 }}>
        <div style={{ maxWidth: "740px", margin: "0 auto" }}>
          <div style={{ display: "flex", gap: "10px", background: "var(--bg-2)", border: "1px solid #2A3F5E", borderRadius: "14px", padding: "8px 8px 8px 16px", alignItems: "flex-end" }}>
            <textarea
              ref={textareaRef}
              value={input}
              ...
              rows={1}
              style={{ flexGrow: 1, background: "none", border: "none", outline: "none", color: "var(--text-1)", fontSize: "14px", resize: "none", fontFamily: "'Space Grotesk', sans-serif", lineHeight: 1.5, paddingTop: "6px", paddingBottom: "6px", maxHeight: "160px", overflowY: "auto" }}
          ...
          <p style={{ color: "var(--text-4)", fontSize: "11px", textAlign: "center", marginTop: "7px" }}>Enter to send · Shift+Enter for new line</p>
        </div>
      </div>
