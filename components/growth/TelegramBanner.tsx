...
type Props = {
  ...
  academy?: string; // pass academy slug for context-aware messaging
};

export function TelegramBanner({ variant = "banner", academy }: Props) {
  const isTech = !academy || ["devops", "cloud", "databases", "ai", "data", "security"].includes(academy);
  const isHealth = academy && ["healthcare", "essentials"].includes(academy);
  const isExam = academy && ["exams", "education"].includes(academy);

  const communityText = isHealth
  ...
  const bannerTitle = isHealth
    ? "Join our Learning Community"
    : isExam
    ? "Join our Exam Prep Community"
  ...
  if (variant === "card") {
    return (
      <a href="https://t.me/synfracore" target="_blank" rel="noopener noreferrer"
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: "15px", marginBottom: "3px" }}>Join our Community</div>
          <div style={{ color: "var(--text-4)", fontSize: "13px" }}>{communityText}</div>
        </div>
  ...
  return (
    <div style={{ background: "linear-gradient(135deg, rgba(0,136,204,0.1), rgba(0,136,204,0.04))", border: "1px solid rgba(0,136,204,0.2)", borderRadius: "16px", padding: "28px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "24px", flexWrap: "wrap" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: "17px", marginBottom: "4px" }}>{bannerTitle}</div>
          <div style={{ color: "var(--text-4)", fontSize: "14px" }}>{communityText}</div>
        </div>
      </div>
      <a href="https://t.me/synfracore" target="_blank" rel="noopener noreferrer"
